...
export function SetPcControlResponse (packet) {
  const buffer = new Uint8Array(packet)
  const view = new DataView(buffer.buffer)
...
export function ActivateKeypadsResponse (packet) {
  const buffer = new Uint8Array(packet)
  const view = new DataView(buffer.buffer)
...
export function SetDoorPasscodesResponse (packet) {
  const buffer = new Uint8Array(packet)
  const view = new DataView(buffer.buffer)
...
export function GetAntipassbackResponse (packet) {
  const buffer = new Uint8Array(packet)
  const view = new DataView(buffer.buffer)
  ...
  return {
    controller: unpackUint32(view, 4),
    antipassback: unpackUint8(view, 8)
  }
}

export function SetAntipassbackResponse (packet) {
  const buffer = new Uint8Array(packet)
  const view = new DataView(buffer.buffer)
